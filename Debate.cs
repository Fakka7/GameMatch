...
namespace GameMatch
{
    public partial class Debate : Form
    {
        ...
        public Debate()
        {
            ...
            this.botonRefrescar.Click += this.botonRefrescar_Click;
        ...
        private void botonRefrescar_Click(object sender, EventArgs e)
        {
            ...
            string consulta = "SELECT Usuario, Juego, Texto FROM debates";
        ...
        private void misDebates_Click(object sender, EventArgs e)
        {
            this.eliminar.Enabled = true;
            this.ver.Enabled = true;
            string usuario = devolverUsuarioPorConexion();
            string connectionString = "Server=LocalHost;Database=gamematch;User ID=root;Password=admin;";

            string consulta = "SELECT Usuario, Juego, Texto FROM debates WHERE usuario = @usuario";
        ...
        public void cargarMisDebates(string usuario)
        {
            ...
            try
            {
                string consulta = "SELECT * FROM debate WHERE usuario = @usuario";
                using (MySqlCommand comando = new MySqlCommand(consulta, conexion))
                {
                    comando.Parameters.AddWithValue("@usuario", usuario);
                    using (MySqlDataAdapter adaptador = new MySqlDataAdapter(comando))
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                using (MySqlConnection conexion2 = new MySqlConnection(connectionString))
                    try
                    {
                        ...
                        string consulta2 = "SELECT Texto FROM debates WHERE Usuario = @Usuario;";
                ...
                {
                    conexion.Open();
                    string consulta = "DELETE FROM debates WHERE Texto = @texto";
                    MySqlCommand comando = new MySqlCommand(consulta, conexion);
        ...
        {
            this.Close();
            Agregar_Debate form2 = new Agregar_Debate();
            form2.Show();
        }
        ...
        private void verDebate_Click(object sender, EventArgs e)
        {
            string valorColumna = "";
            if (dataGridView1.SelectedRows.Count > 0)
            {
            ...
            this.Close();
            VerDebate form2 = new VerDebate(valorColumna); 
            form2.Show(); 
        }
    }
}
